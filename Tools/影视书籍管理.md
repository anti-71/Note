...
function renderGallery() {
    ...
    filteredPages.forEach(page => {
        ...
        const match = fileContent.match(frontmatterRegex);
                ...
                if (match) {
            // 有 frontmatter
            let frontmatter = match[1];  // 不包含前后的 '---'
            const body = match[2];
            
            // 处理 status 字段
            if (frontmatter.includes('status:')) {
                frontmatter = frontmatter.replace(/status:\s*(true|false)/i, `status: ${newStatus}`);
            } else {
                frontmatter += `status: ${newStatus}\n`;
            }
            
            // 处理“观毕时间”字段
            if (frontmatter.includes('观毕时间:')) {
                frontmatter = frontmatter.replace(/观毕时间:\s*\S+/, `观毕时间: ${todayStr}\n`);
            } else {
                frontmatter += `观毕时间: ${todayStr}\n`;
            }
