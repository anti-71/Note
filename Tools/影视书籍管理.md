    ...
    filteredPages.forEach(page => {
        ...
        statusTag.addEventListener('click', async (event) => {
            ...
            try {
                ...
                // 使用正则分离 frontmatter 和正文（支持 frontmatter 标准格式）
                ...
                if (match) {
    				...
    				// 将 frontmatter 按行拆分处理，避免正则跨行误删
    				let lines = frontmatter.split('\n');
    
				    // 1. 处理 status 字段
				    let statusIndex = lines.findIndex(l => l.trim().startsWith('status:'));
				    if (statusIndex !== -1) {
				        lines[statusIndex] = `status: ${newStatus}`;
				    } else {
				        lines.push(`status: ${newStatus}`);
				    }
				    
				    // 2. 处理 观毕时间 字段 (精准替换，不吞掉换行和下一行)
				    let timeIndex = lines.findIndex(l => l.trim().startsWith('观毕时间:'));
				    if (timeIndex !== -1) {
				        lines[timeIndex] = `观毕时间: ${todayStr}`;
				    } else {
				        lines.push(`观毕时间: ${todayStr}`);
				    }
    
				    // 重新组合，过滤掉可能产生的空行并用换行符连接
