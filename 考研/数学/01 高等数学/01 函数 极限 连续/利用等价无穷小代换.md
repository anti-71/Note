---
名称: 利用等价无穷小代换
章: 第一章 函数 极限 连续
节: "[[第二节 极限]]"
tags:
  - 知识点
index: 17
---
##### 1）代换原则

若 $\alpha\sim\alpha_1$，$\beta\sim\beta_1$

乘除关系可换：

$$\lim\frac{\alpha}{\beta}=\lim\frac{\alpha_1}{\beta}=\lim\frac{\alpha}{\beta_1}=\lim\frac{\alpha_1}{\beta_1}$$

加减关系在一定条件下可换：

- 若 $\lim\dfrac{\alpha_1}{\beta_1}=A\neq1$，则 $\alpha-\beta\sim\alpha_1-\beta_1$
- 若 $\lim\dfrac{\alpha_1}{\beta_1}=A\neq-1$，则 $\alpha+\beta\sim\alpha_1+\beta_1$

##### 2）常用的等价无穷小

当 $x\to0$ 时，

$$x\sim sinx\sim tanx\sim arcsinx\sim arctanx\sim ln(1+x)\sim e^x-1$$

$$(1+x)^{\alpha}-1\sim\alpha x(\alpha\neq0),\  1-cosx\sim\frac{1}{2}x^2,\ \alpha^x-1\sim xlna$$

$$x-sinx\sim\frac{1}{6}x^3,\tan x-x\sim\frac{1}{3}x^3,\  x-ln(1+x)\sim\frac{1}{2}x^2$$

$$arcsinx-x\sim\frac{1}{6}x^3,\  x-arctanx\sim\frac{1}{3}x^3$$

- 当 $x\to1$，$lnx\to x-1$
	
	当 $x\to0$ 时, $(1+x)^{\alpha}-1\sim\alpha x\ (\alpha\neq0)$ 的推广：
	  
- 若 $\alpha(x)\to0,\alpha(x)\cdot \beta(x)\to0$，则 $(1+\alpha(x))^{\beta(x)}-1\sim\alpha(x)\cdot \beta(x)$
	- 设 $f(x)$ 和 $g(x)$ 在 $x=0$ 的某邻域内连续，且 $\lim\limits_{x\to 0}\dfrac{f(x)}{g(x)}=1$，则
	  $$
	  \int_{0}^{x}f(t)\mathrm{d}t \sim \int_{0}^{x}g(t)\mathrm{d}t
	  $$

> [!note] 
> 
> 特别的，
> 
> 1. 若 $f(x)\sim g(x)\implies\displaystyle\int^x_0f(t)dt\sim\int^x_0g(t)dt$
> 
> 2. 若 $f(x)\to A$，$g(x)\to A$ 且 $\displaystyle A\neq0\implies\int^x_0f(t)dt\sim\int^x_0g(t)dt$

> [!warning] 
> 复合函数使用等价无穷小代换，代换的部分的极限一定要存在，不然代换非法