...
$$
存在，则称$f(x)$**在点$x_0$处可导**，并称此极限值为$f(x)$**在点$x_0$处的导数**，记为$f'(x_0)$，或$y'|_{x=x_0}$，或$\frac{dy}{dx}|_{x=x_0}$
...
**定义**	若$y=f(x)$在$(a,b)$内每一点都可导，则称$f(x)$**在区间$(a,b)$内可导**

此时对于$(a,b)$内每一点x，都对应一个导数值$f'(x)$，常称$f'(x)$为$f(x)$在$(a,b)$内的**导函数**，简称**导数**
...
> 设函数$y = \frac{1}{2x + 3} $，则$ y^{(n)}(0) = \underline{\quad\quad} $

已知$\frac{1}{1 + x}$的幂级数展开式：  
$$
\frac{1}{1 + x} = 1 - x + x^2 + \cdots + (-1)^n x^n + o(x^n)
$$
对函数$ y = \frac{1}{2x + 3} $变形并展开：  
$$
\begin{align*}  y &= \frac{1}{2x + 3} \\  &= \frac{1}{3} \cdot \frac{1}{1 + \frac{2}{3}x} \\  &= \frac{1}{3} \left[ 1 - \frac{2}{3}x + \cdots + (-1)^n \left( \frac{2}{3}x \right)^n \right] + o(x^n) \\  &= \frac{1}{3} - \frac{2}{3^2}x + \cdots + \frac{(-1)^n 2^n}{3^{n+1}} x^n + o(x^n)  \end{align*}
$$
根据幂级数展开与 $ n $ 阶导数的关系，对比 $ x^n $ 项系数可得： 
$$
 \frac{y^{(n)}(0)}{n!} = \frac{(-1)^n 2^n}{3^{n+1}}
$$
因此，$ n $ 阶导数在 $ 0 $ 处的值：  
$$
